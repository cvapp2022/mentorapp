 ...
 */
function generateSlots(start, end, interval, randomSlotsToDelete, except) {
  ...
  for (; startStamp <= endStamp; startStamp += interval * 60000) {
    ...
    if(!except.includes(new Date(slot.date).getTime())){
      slots.push(slot);
    }
  }
...
function generateFirstDate(date, interval, startTime, endTime) {
  ...
  const slots = generateSlots(start, end, interval,0,[]);
  return {
    date: roundDate(date),
    slots,
  };
 ...
 */
function generateDays(date, nbDays, startTime, endTime, interval, randomSlotsToDelete = 0, except = []) {
  const days = [];
  console.log('except dates from slot function', except)
